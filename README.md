...
- **自动敏感信息脱敏**：分享链接 / 导出 HTML 时自动检测并打码 API Key、Token（OpenAI/Anthropic/GitHub/npm/AWS/Slack/JWT/Bearer）、密码、私钥、连接串凭据——你随手贴在对话里的 key 不会随链接外发，无需人工逐条检查（本机 1:1 回放保留原文）。
